...
ポイント
- Durable Objects は `wrangler.toml` の `[[durable_objects.bindings]]` と `[[migrations]]` により自動でクラス作成・マイグレーションされる（初回デプロイ時）。
- Free プランでは `new_sqlite_classes = ["RoomDurable"]` が必須（従来の `new_classes` ではエラー 10097 が発生）。本リポジトリは sqlite DO で設定済み。
